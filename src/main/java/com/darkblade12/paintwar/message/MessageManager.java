...
public class MessageManager extends Manager implements MessageContainer {
		...
		try {
			...
			while (line != null) {
				...
				if (s.length == 2 && !line.startsWith("#"))
					// Keep messages with & color codes for legacy support
					messages.put(s[0], s[1].replace(";", "\n"));
	...
	/**
	 * Converts a message string (with & color codes) to an Adventure Component.
	 * This is used for modern chat formatting on Paper 1.13+.
	 */
	public Component toComponent(String message) {
		return ColorCodeUtil.fromLegacyString(message);
	}
}
