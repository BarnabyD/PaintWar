...
public class Arena extends MultipleTaskManager {
		...
		if (ignoredBlockIdsString != null)
			for (String block : ignoredBlockIdsString.split(", "))
				try {
					Material mat = null;
					try {
						// Try to parse as material name first (new format)
						mat = Material.getMaterial(block.toUpperCase());
					} catch (Exception e1) {
						// Fall back to numeric ID parsing for old configs
						try {
							int id = Integer.parseInt(block);
							// In modern Bukkit, we can't get material by ID, so we'll skip old numeric IDs
							plugin.l.warning("Numeric block IDs are no longer supported. Please use material names in config. Skipping block ID: " + id);
							continue;
						} catch (NumberFormatException e2) {
							throw new Exception("Invalid block format: " + block);
						}
					}
					if (mat == null || !mat.isBlock())
						throw new Exception("Invalid block material '" + block + "' found in the list 'Ignored_Block_Ids'");
	...
	public void broadcastMessage(String message, Player... excepted) {
		...
		if (players.size() != exceptedNames.size()) {
			net.kyori.adventure.text.Component component = plugin.message.toComponent(message);
			for (Player p : getPlayers())
	...
	public void distributeRewards(Player p) {
		...
		if (moneyRewardEnabled && plugin.vault != null) {
			// depositPlayer is deprecated but no replacement available in Vault
			@SuppressWarnings("deprecation")
			var unused = plugin.vault.eco.depositPlayer(p.getName(), moneyAmount);
	...
	private void broadcastMessageToServer(String msg) {
		net.kyori.adventure.text.Component component = plugin.message.toComponent(msg);
