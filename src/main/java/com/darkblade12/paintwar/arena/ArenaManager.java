...
public class ArenaManager extends Manager implements Listener {
	...
	private void checkIllegalPlayerAction(Cancellable event, Player p, Location loc, String action) {
		...
		if (a != null) {
			if (!p.hasPermission("PaintWar.build." + a.getName()) && !p.hasPermission("PaintWar.build.*") && !p.hasPermission("PaintWar.*")) {
				...
				ColorCodeUtil.sendColoredMessage(p, plugin.message.arena_action_not_allowed(action));
			} else if (!a.isInEditMode()) {
				event.setCancelled(true);
				ColorCodeUtil.sendColoredMessage(p, plugin.message.arena_not_in_edit_mode());
			}
		}
	}
	...
	public void onProjectileHit(ProjectileHitEvent event) {
		...
		if (!(pr instanceof Snowball))
			return;
		Object shooter = pr.getShooter();
		...
		loc.getWorld().playSound(loc, Sound.ENTITY_SLIME_JUMP, 1.0F, 5.0F);
	...
	public void onPlayerChat(AsyncPlayerChatEvent event) {
		...
		if (a != null && event.getMessage().equalsIgnoreCase("ready")) {
			...
			if (a.getState() != State.JOINABLE) {
				ColorCodeUtil.sendColoredMessage(p, plugin.message.arena_game_x_started(true));
			} else if (plugin.data.isReady(p)) {
				ColorCodeUtil.sendColoredMessage(p, plugin.message.arena_already_ready());
			} else {
				plugin.data.setReady(p, true);
				ColorCodeUtil.sendColoredMessage(p, plugin.message.arena_ready());
	...
	public void onPlayerCommand(PlayerCommandPreprocessEvent event) {
		...
		if (a != null) {
			...
			if (!a.isCommandAllowed(cmd.replace("/", ""))) {
				event.setCancelled(true);
				ColorCodeUtil.sendColoredMessage(p, plugin.message.arena_action_not_allowed(plugin.message.getMessage("action_use_command").replace("<command>", cmd.split(" ")[0])));
			}
		}
	}
	...
	public void onPlayerInteract(PlayerInteractEvent event) {
		Player p = event.getPlayer();
		ItemStack h = p.getEquipment().getItemInMainHand();
		...
		if (a.name().contains("BLOCK")) {
			...
			ColorCodeUtil.sendColoredMessage(p, plugin.message.player_position_set(p, first, loc));
		}
	}
