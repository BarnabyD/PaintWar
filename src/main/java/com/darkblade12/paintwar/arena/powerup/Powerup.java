...
public enum Powerup {
	...
	SPEED("Speed", Material.DIAMOND_BOOTS) {
		...
		public void activate(final PaintWar plugin, final Player p, final PowerupManager manager, final Arena a) {
			...
			p.addPotionEffect(new PotionEffect(PotionEffectType.SPEED, duration, settings[0], true));
			final Powerup pow = this;
			int task = manager.scheduleTask(new Runnable() {
				@Override
				public void run() {
	...
	BIG_BLOB("Big_Blob", Material.FIREWORK_STAR) {
		@Override
		public void activate(final PaintWar plugin, final Player p, final PowerupManager manager, final Arena a) {
			super.activate(plugin, p, manager, a);
	...
	ADVANCED_DARKNESS("Advanced_Darkness", Material.ENDER_EYE) {
		@Override
		public void activate(final PaintWar plugin, final Player p, final PowerupManager manager, final Arena a) {
			super.activate(plugin, p, manager, a);
			final String name = p.getName();
	...
	DRUNKEN("Drunken", Material.POTION) {
		...
		public void activate(final PaintWar plugin, final Player p, final PowerupManager manager, final Arena a) {
			...
			int[] settings = manager.getSettings(this);
			final int duration = settings[1] * 20;
			PotionEffect effect = new PotionEffect(PotionEffectType.NAUSEA, duration, settings[0]);
			for (Player ap : a.getPlayers())
				if (!ap.getName().equals(name))
					ap.addPotionEffect(effect);
			final Powerup pow = this;
	...
	SLOWNESS("Slowness", Material.COAL) {
		...
		public void activate(final PaintWar plugin, final Player p, final PowerupManager manager, final Arena a) {
			...
			int[] settings = manager.getSettings(this);
			final int duration = settings[1] * 20;
			PotionEffect effect = new PotionEffect(PotionEffectType.SLOWNESS, duration, settings[0]);
			for (Player ap : a.getPlayers())
				if (!ap.getName().equals(name))
					ap.addPotionEffect(effect);
			final Powerup pow = this;
	...
	COLOR_BOMBS("Color_Bombs", Material.SLIME_BALL) {
		...
		public void activate(final PaintWar plugin, final Player p, final PowerupManager manager, final Arena a) {
			...
			final String name = p.getName();
			int[] settings = manager.getSettings(this);
			p.getInventory().addItem(new ItemStack(Material.SNOWBALL, settings[1]));
			final Powerup pow = this;
			int task = manager.scheduleTask(new Runnable() {
				@Override
				public void run() {
	...
	DASH("Dash", Material.FIREWORK_ROCKET) {
		@Override
		public void activate(final PaintWar plugin, final Player p, final PowerupManager manager, final Arena a) {
			super.activate(plugin, p, manager, a);
			final String name = p.getName();
