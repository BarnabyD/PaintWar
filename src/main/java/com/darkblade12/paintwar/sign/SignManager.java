...
public class SignManager extends SingleTaskManager implements Listener {
	...
	public void onSignChange(SignChangeEvent event) {
		...
		if (!p.hasPermission("PaintWar.sign.create") && !p.hasPermission("PaintWar.*") && !p.hasPermission("PaintWar.sign.*")) {
			b.breakNaturally();
			ColorCodeUtil.sendColoredMessage(p, plugin.message.sign_no_x_permission(true));
			return;
		}
		Arena a = null;
		for (int i = 1; i < 4; i++) {
		...
		if (a == null) {
			b.breakNaturally();
			ColorCodeUtil.sendColoredMessage(p, plugin.message.sign_no_arena_found());
			return;
		}
		...
		new BukkitRunnable() {
			...
			public void run() {
				...
				ColorCodeUtil.sendColoredMessage(p, plugin.message.sign_x(true, as.getId()));
			}
	...
	public void onBlockBreak(BlockBreakEvent event) {
		...
		if (as == null)
			...
			ColorCodeUtil.sendColoredMessage(p, plugin.message.sign_x(false, as.getId()));
			return;
		}
		event.setCancelled(true);
		ColorCodeUtil.sendColoredMessage(p, plugin.message.sign_no_x_permission(false));
	}
}
