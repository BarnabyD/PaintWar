...
public class CommandDetails {
	...
	public boolean checkUsage(PaintWar plugin, CommandSender s, String[] args) {
		if (!(s instanceof Player) && !executableAsConsole) {
			ColorCodeUtil.sendColoredMessage(s, plugin.message.command_no_console_executor());
			return false;
		} else if (!hasPermission(s)) {
			ColorCodeUtil.sendColoredMessage(s, plugin.message.command_no_permission());
			return false;
		}
		int min = 0;
		int max = 0;
		...
		if (args.length < min || args.length > max) {
			ColorCodeUtil.sendColoredMessage(s, getInvalidUsageString());
			return false;
		}
		return true;
	}
